...
<?php include template("content_inc","position"); ?>
  <div class="row">
    <div class="col xs-12 md-8">
      ...
      <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=79e50c74971829298d740db1899110e9&sql=SELECT+A.username%2CB.%2A+FROM+v9_member+as+A+left+join+v9_member_detail+as+B+on+A.userid+%3D+B.userid+where+A.modelid+%3D+10&cache=3600&page=%24page&num=2&return=data\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$pagesize = 2;$page = intval($page) ? intval($page) : 1;if($page<=0){$page=1;}$offset = ($page - 1) * $pagesize;$r = $get_db->sql_query("SELECT COUNT(*) as count FROM (SELECT A.username,B.* FROM v9_member as A left join v9_member_detail as B on A.userid = B.userid where A.modelid = 10) T");$s = $get_db->fetch_next();$pages=pages($s['count'], $page, $pagesize, $urlrule);$r = $get_db->sql_query("SELECT A.username,B.* FROM v9_member as A left join v9_member_detail as B on A.userid = B.userid where A.modelid = 10 LIMIT $offset,$pagesize");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
      <ul class="bts lbs">
        <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
        <li>
          <a href="index.php?m=content&c=index&a=member_home&catid=<?php echo $catid;?>&userid=<?php echo $r['userid'];?>" target="_blank" class="card">
